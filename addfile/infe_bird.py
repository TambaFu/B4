...
model_config_path = "config/DINO/DINO_4scale_faster_vit_0_224_bird.py" # change the path of the model config file
model_checkpoint_path = "checkpoint/checkpoint_best_voc" # change the path of the model checkpoint
# See our Model Zoo section in README.md for more details about our pretrained models.
#config ファイルに含まれる情報（例: 学習率、エポック数、モデル構造など）が辞書形voc式で args へ
...
with open('util/bird_id2name.json') as f:
    id2name = json.load(f)
    id2name = {int(k):v for k,v in id2name.items()}
#args（辞書オブジェクト）に新しいキーと値を追加
args.dataset_file = 'bird'
args.coco_path = "bird" # the path of coco
args.fix_size = False
...
thershold = 0.33 # set a thershold
